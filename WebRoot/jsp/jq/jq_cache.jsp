...
/**
  ps:
	  	...
	  	$('#catchDiv').data('data',data);  这是缓存功能代码 .data(key,value); 取值得时候,通过key来取
	  	第二次加载数据源时url改为load1.jsp,该地址是不存在的,所以执行加载时会发生错误 
	  	这时在error中就把缓存数据拿出来使用
 */
$(function(){
		var num = 0;
		var url = 'jsp/jq/load.jsp';
		$('#loadBtn').click(function(){
			
			$('#loadingDiv').html('正在奋力加载中......');
			
			if(num > 0){
				url = 'jsp/jq/load1.jsp';
			}
...
<div id="catchDiv"></div>
<input type="button" id="loadBtn" value="加载页面"/><a href="index.jsp">返回首页</a>
</body>
</html>